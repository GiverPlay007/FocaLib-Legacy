...
public class ChannelManager {
	...
	public void createPermanentChannel(Channel c) {
		if(existsChannel(c.getName()))
			return;
		channels.put(c.getName().toLowerCase(), c);
		File channel = new File(LegendchatAPI.getPlugin().getDataFolder(),"channels"+File.separator+c.getName().toLowerCase()+".yml");
		if(!channel.exists()) {
	...
	public void deleteChannel(Channel c) {
		...
		new File(LegendchatAPI.getPlugin().getDataFolder(),"channels"+File.separator+c.getName().toLowerCase()+".yml").delete();
	}
	...
	public void loadChannels() {
		String bungee = LegendchatAPI.getPlugin().getConfig().getString("bungeecord.channel");
		channels.clear();
		for (File channel : new File(LegendchatAPI.getPlugin().getDataFolder(),"channels").listFiles()) {
			if(channel.getName().toLowerCase().endsWith(".yml")) {
		        if(!channel.getName().toLowerCase().equals(channel.getName()))
		        	channel.renameTo(new File(LegendchatAPI.getPlugin().getDataFolder(),"channels"+File.separator+channel.getName().toLowerCase()));
