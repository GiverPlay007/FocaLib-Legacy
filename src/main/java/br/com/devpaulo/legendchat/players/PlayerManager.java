...
public class PlayerManager
{
	...
	public void playerDisconnect(Player p)
	{
		...
		if (!LegendchatAPI.maintainSpyMode())
		{
			removeSpy(p);
		}
	...
	{
		if (LegendchatAPI.getDefaultChannel()!=c && c!=null)
		{
			if (!p.hasPermission("legendchat.channel." + c.getName() + ".focus") && !p.hasPermission("legendchat.admin"))
			{
				if (msg)
				{
					p.sendMessage(LegendchatAPI.getMessageManager().getMessage("error5"));
				}
				return;
			}
		}
		...
		if (msg)
		{
			p.sendMessage(LegendchatAPI.getMessageManager().getMessage("message1").replace("@channel", c.getName()));
		}
	}
