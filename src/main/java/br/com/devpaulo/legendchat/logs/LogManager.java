...
public class LogManager {
	...
	public void startSavingScheduler() {
		Bukkit.getScheduler().runTaskTimer(LegendchatAPI.getPlugin(), new Runnable() {
			public void run() {
				saveLog();
			}
		}, LegendchatAPI.getLogToFileTime()*1200, LegendchatAPI.getLogToFileTime()*1200);
	}
	...
	private class Executor extends Thread {
		...
		public void run() {
			File f2 = new File(LegendchatAPI.getPlugin().getDataFolder(),"logs");
			if(!f2.exists())
				f2.mkdir();
			HashMap<String,List<Log>> date_log = new HashMap<String,List<Log>>();
			for(Log l : saving_log) {
				String n = getFileName(l.getDate());
				File f = new File(LegendchatAPI.getPlugin().getDataFolder()+File.separator+"logs",n);
				if(!f.exists()) {
